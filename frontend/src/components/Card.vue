<template>
  <div class="card shadow" style="width: 12rem; min-height: 10rem">
    <h5 class="card-header" :class="{ 'bg-success': done, 'bg-danger': deadpass }">
      {{ title }}
    </h5>
    <div class="card-body">
    ...
    <div class="card-footer bg-white">
      <div class="col">
        <i @click="editCard(card_id)" class="bi bi-pencil text-primary h5 mx-2" data-toggle="tooltip" data-placement="top"
          title="Edit"></i>

        <i @click="deleteCard(card_id)" class="bi bi-trash text-danger h5 mx-2" data-toggle="tooltip" data-placement="top"
          title="Delete"></i>
        <i @click="exportCard(card_id)" class="bi bi-file-earmark-arrow-down-fill text-success h5 mx-2"
          data-toggle="tooltip" data-placement="top" title="Export"></i>
      </div>
      <div class="col">
        <!-- Default switch -->
        <div class="form-switch">
          <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault"
            @click="completetoggleSwitch" :checked="checked" />

          <label class="mx-2 form-check-label" for="flexSwitchCheckDefault">Done</label>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    deleteCard: function (id) {
      if (confirm("Are you sure you want to delete this card")) {
        fetch(`http://127.0.0.1:5000/api/card/${id}`, {
          method: "DELETE",
          headers: {
    ...
    completetoggleSwitch: function () {
      fetch(`http://127.0.0.1:5000/api/card/complete/${this.card_id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Access-Control-Allow-Origin": "*",
