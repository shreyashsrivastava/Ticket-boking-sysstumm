<template>
  <div class="card shadow" style="width: 15rem; min-height: 10rem">
    <div class="card-header bg-warning">
      <h5>{{ title }}</h5>
      <div class="col">
        <i @click="editList(id)" class="bi bi-pencil text-primary h5 mx-2" data-toggle="tooltip" data-placement="top"
          title="Edit"></i>

        <i @click="deleteList(id)" class="bi bi-trash text-danger h5 mx-2" data-toggle="tooltip" data-placement="top"
          title="Delete"></i>
        <i @click="exportList()" class="bi bi-file-earmark-arrow-down-fill text-success h5 mx-2" data-toggle="tooltip"
          data-placement="top" title="Export"></i>
        <i @click="addCard(id)" class="bi bi-plus-square-fill h5 mx-2" data-toggle="tooltip" data-placement="bottom"
          title="Add Card"></i>
      </div>
    </div>
    <div class="card-body" v-for="card in allcards" :key="card.id">
      <Card :card_id="card.id" :content="card.content" :deadline="card.deadline" :title="card.title"
        :completed="card.completed" :deadline_days="card.deadline_days" @delete-card="deletecard"></Card>
...
export default {
  ...
  methods: {
    deleteList: function (id) {
      ...
      if (confirm("Do you really want to delete this list")) {
        fetch(`http://127.0.0.1:5000/api/list/${this.id}`, {
          method: "DELETE",
          headers: {
  ...
  mounted: function () {
    ...
    fetch(`http://127.0.0.1:5000/api/card/list/${this.id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("access_token"),
